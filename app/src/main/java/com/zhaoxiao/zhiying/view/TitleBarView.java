...
public class TitleBarView extends RelativeLayout {
    ...
    public TitleBarView(final Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (attr) {
                case R.styleable.TitleBarView_tb_leftTextColor:
                    tv_left.setTextColor(array.getColor(attr, Color.BLACK));
                    break;
                case R.styleable.TitleBarView_tb_leftDrawable:
                    iv_left.setImageResource(array.getResourceId(attr, 0));
                    break;
                case R.styleable.TitleBarView_tb_leftText1:
                    tv_left.setText(array.getString(attr));
                    break;
                case R.styleable.TitleBarView_tb_centerTextColor:
                    tv_title.setTextColor(array.getColor(attr, Color.BLACK));
                    break;
                case R.styleable.TitleBarView_tb_centerTitle:
                    tv_title.setText(array.getString(attr));
                    break;
                case R.styleable.TitleBarView_tb_rightDrawable:
                    iv_right.setImageResource(array.getResourceId(attr, 0));
                    break;
                case R.styleable.TitleBarView_tb_rightText:
                    tv_right.setText(array.getString(attr));
                    break;
                case R.styleable.TitleBarView_tb_rightTextColor:
